		...
		if ("internal 1", "internal 2", "monitor", "phone")
			...
			if (A && isAI(A))
				...
				if (A.radio3 && istype(A.radio3, /obj/item/device/radio/))
					R3 = A.radio3
				if (A.internal_phone && istype(A.internal_phone, /obj/machinery/phone/ai))
					R4 = A.internal_phone
			...
			switch (message_mode)
				...
				if ("phone")
					if (R4 && !(A.stat || A.hasStatus(list("stunned", "weakened"))))
						skip_open_mics_in_range = 1
						if (R4.handset)
							R4.handset.talk_into(src, messages, secure_headset_mode, A.name, lang_id)
							italics = 1
							DEBUG_MESSAGE("AI internal phone triggered. Message: [message]")
						else
							src.show_text("Notice: No connected caller.", "red")
					else
